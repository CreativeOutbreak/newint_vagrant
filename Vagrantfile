...
Vagrant.configure('2') do |config|
  ...
  # New Internationalist shared folder
  config.vm.synced_folder "nidata", "/vagrant_nidata"
  ...
  # New Internationalist Provisioning
 

  # Setup drupal.dev vhost and drupal7 directory
  #config.vm.provision "shell", path: "scripts/ni/sites-available.sh"

  # Run legacy db setup shell script.
  #config.vm.provision "shell", path: "scripts/ni/postgresql.sh"


  # Run legacy db setup shell script.
  #config.vm.provision "shell", path: "scripts/ni/legacy_db.sh"

  # Run modulesetup shell script.
  #config.vm.provision "shell", path: "scripts/ni/d7instal.sh"

  # Run modulesetup shell script.
  #config.vm.provision "shell", path: "scripts/ni/d7modules.sh"
